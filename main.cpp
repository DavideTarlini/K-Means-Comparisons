...
void generate_points(int n, int dim, int numClusters, std::vector<std::vector<double>> &points) {
...
void get_initial_centroids(int n, int dim, const int k, const std::vector<std::vector<double>> &points, std::vector<std::vector<double>> &centroids){
            ...
            for(int y = 0; y<sizeof(dimensions)/sizeof(dimensions[0]); y++){
                ...
                for(int x = 0; x<sizeof(num_points)/sizeof(num_points[0]); x++){
                    ...
                    for(int w = 0; w<runs; w++){
                        ...
                        generate_points(n, d, k, points);
                        get_initial_centroids(n, d, k, points, init_centroids);
                        ...
                        auto c_seq_simd = kmeans_seq_simd(k, points, init_centroids);
                         ...
                         *  */ 
                        t1 = std::chrono::high_resolution_clock::now();
                        auto c_par_crit = kmeans_par_crit(k, points, init_centroids);
                        t2 = std::chrono::high_resolution_clock::now();
                        std::chrono::duration<double, std::milli> ms_par_crit = t2 - t1;
                        //std::cout << ms_par.count() << "ms       par\n";
                        for (size_t h = 0; h < k; h++)
                        {
                            for (size_t l = 0; l < dimensions[y]; l++)
                            {
                                std::cout << c_par_crit[h][l] << " ";
                            }
                            std::cout << "\n"; 
                        }
                        std::cout << "\n --par atomic-- \n\n";
                        ...
                        t1 = std::chrono::high_resolution_clock::now();
                        auto c_par_simd = kmeans_par_simd(k, points, init_centroids);
                        t2 = std::chrono::high_resolution_clock::now();
                        ...
                        /* Speedups */
                        speed_res[0][w] = safe_div(ms_seq.count(), ms_seq_simd.count());
                        speed_res[1][w] = safe_div(ms_seq.count(), ms_par_crit.count());
                        speed_res[2][w] = safe_div(ms_seq.count(), ms_par.count());
                        speed_res[3][w] = safe_div(ms_seq.count(), ms_par_simd.count());
